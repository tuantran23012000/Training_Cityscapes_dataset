...
def convert_coco_json(json_dir='../coco/annotations/', use_segments=False, cls91to80=False):
    ...
    for json_file in sorted(Path(json_dir).resolve().glob('*.json')):
        ...
        for img_id, anns in tqdm(imgToAnns.items(), desc=f'Annotations {json_file}'):
            ...
            img_file = os.path.join(os.getcwd()+"/data/leftImg8bit_trainvaltest",f)
            label_file = os.path.join(os.getcwd()+"/data","gtFine"+f[11:])[:-3]+"txt"
            p_img_train = os.path.join(os.getcwd(),"images/train/")
            p_img_val = os.path.join(os.getcwd(),"images/val/")
            p_img_test = os.path.join(os.getcwd(),"images/test/")
            p_label_train = os.path.join(os.getcwd(),"labels/train/")
            p_label_val = os.path.join(os.getcwd(),"labels/val/")
            p_label_test = os.path.join(os.getcwd(),"labels/test/")
            img_name = f.split("/")[-1]
            label_name = f.split("/")[-1][:-3] + "txt"
            # print(img_name)
            # print(label_name)
            if "/train/" in img_file:
                #if not os.path.exists(p_img_train):

                os.makedirs(p_img_train,exist_ok=True)
                os.makedirs(p_label_train,exist_ok=True)
                shutil.copy(img_file,os.path.join(p_img_train,img_name))
                with open(os.path.join(p_label_train,label_name), 'a') as file:
                    for i in range(len(bboxes)):
                        line = *(segments[i] if use_segments else bboxes[i]),  # cls, box or segments
                        file.write(('%g ' * len(line)).rstrip() % line + '\n')
                with open("train.txt", 'a') as file_:
                    file_.write(os.path.join("./images/train/",img_name)+'\n')

            elif "/val/" in img_file:
                #if not os.path.exists(p_img_val):
                os.makedirs(p_img_val,exist_ok=True)
                os.makedirs(p_label_val,exist_ok=True)
                shutil.copy(img_file,os.path.join(p_img_val,img_name))
                with open(os.path.join(p_label_val,label_name), 'a') as file:
                    for i in range(len(bboxes)):
                        line = *(segments[i] if use_segments else bboxes[i]),  # cls, box or segments
                        file.write(('%g ' * len(line)).rstrip() % line + '\n')
                with open("val.txt", 'a') as file_:
                    file_.write(os.path.join("./images/val/",img_name)+'\n')
            elif "/test/" in img_file:
                #if not os.path.exists(p_img_test):
                os.makedirs(p_img_test,exist_ok=True)
                os.makedirs(p_label_test,exist_ok=True)
                shutil.copy(img_file,os.path.join(p_img_test,img_name))
                with open(os.path.join(p_label_test,label_name), 'a') as file:
                    for i in range(len(bboxes)):
                        line = *(segments[i] if use_segments else bboxes[i]),  # cls, box or segments
                        file.write(('%g ' * len(line)).rstrip() % line + '\n')
                with open("test.txt", 'a') as file_:
                    file_.write(os.path.join("./images/test/",img_name)+'\n')
...
if __name__ == '__main__':
    ...
    if source == 'COCO':
        convert_coco_json('/home/tuantran/Documents/CMC/cityscapes/data/annotations',  # directory with *.json
                          use_segments=False,
                          cls91to80=True)